...
//      const dripbot = await fetcher(`https://api.thedripbot.com/dripbot/drip/cryptodads/?token_id=${tokenId}&accessory=2Clickz`)
//      return dripbot;
//    } 
...
async function getResponse(req: NextRequest): Promise<NextResponse> {
  ...
  const url = new URL("https://", `http://${req.headers.get('host')}`);
  dadid = url.searchParams.get('dadid');
  ...
  return new NextResponse(
    getFrameHtmlResponse({
      ...
      postUrl: `https://steak.cryptodadsnft.com/api/dadframe?dadid=${dadid}&trait=${trait}`,
    }),
  );
}
